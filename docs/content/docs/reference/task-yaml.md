...
resources:
  cloud: aws                # aws, gcp, azure, kubernetes
  region: us-east-1
  zone: us-east-1a
...
| `workdir` | string | | Working directory path or git URL |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `cloud` | string | any | Cloud provider: `aws`, `gcp`, `azure`, `kubernetes` |
...
| `image_id` | string | | VM image ID or `docker:<image>` |
